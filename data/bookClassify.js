...
let bookClas = mongoose.model('bookClassify', classifySchema)
...
let xuanhuanUrl = '../data/classify-xuanhuan.html'
let xiuzhenUrl = '../data/classify-xiuzhen.html'

saveBook(xuanhuanUrl, 2)
  .then(function(data) {
    data.forEach(item => {
      new bookClas(item).save(function(err, result) {
        if (err) {
          console.log(err)
        }
      })
